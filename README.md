...
- The console settings lane now also emits a single replacement-query contract readiness summary so operators can tell at a glance whether the live descriptor plus both wire schemas are all available for client-owned employee UIs.
- The console settings lane now also emits a single durable-worker readiness summary so operators can tell at a glance whether the shared worker startup contract is ready before trusting ingestion, retention, or evaluation jobs.
...
- those Kubernetes/OpenShift external-query examples now also keep `/` routed only on the
  console host; the employee host exposes `/v1` and `/health` only so the package stays
  API-only for client-owned chat shells
