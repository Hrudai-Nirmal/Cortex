...
- The developer console settings lane now also emits a single replacement-query contract readiness summary, so operators can tell whether the live descriptor plus both wire schemas are all available without mentally reconciling separate handshake and schema rows.
- The developer console settings lane now also emits a single durable-worker readiness summary, so operators can see whether the shared worker startup contract is ready before trusting background ingestion, retention, or evaluation processing.
- The developer console operator release gate now also fails closed when those live replacement-query schemas are missing, so a package is not treated as client-shippable unless third-party employee UIs can validate both the contract descriptor and the request/response wire shape.
...
- That live external-query descriptor now also points at machine-readable request/response JSON Schema endpoints (`/v1/chat/contracts/v1/schemas/request` and `/v1/chat/contracts/v1/schemas/response`), so client-owned chat shells can validate their wire contract against the running Cortex package instead of relying only on prose docs or a full OpenAPI dump.
...
- Development fixture seeding now also bypasses live embeddings entirely when sample traces are disabled, so the free local test corpus can be loaded even when the local model endpoint lacks the OpenAI-style embeddings route.
- The deterministic extractive claim path now gates exact citation-backed answers on the strongest evidence-quality signal (`sourceScore` vs. blended support), which keeps the CPU-friendly packaged profile fast on straightforward evidence-backed questions instead of spending minutes in local generation.
- The packaged edge healthchecks now probe a routed console host header instead of the unrouted bare root, so split-host Docker and ECS deployments report the proxy healthy instead of failing on intentional host-based `404` responses.
...
- The fixed console left navigation now drives the same shared workspace tab state as the in-surface tab strip, and it only exposes real shipped console sections instead of dead placeholder entries, so operator navigation remains truthful and clickable.
...
- Package operator tooling now also treats the live request/response schema endpoints as part of that same boundary: `package:verify` asserts the exported schema paths plus the core request/response JSON Schema shape, and `package:status` prints compact schema summaries so operators can confirm what a client-owned query UI is expected to send and receive.
...
- Package bootstrap, status, and verification now also assert `X-Cortex-Surface: query` in
  that external mode, so operators can tell a deliberate API-only query host from a generic
  proxy/default-host `404`.
