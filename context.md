...
- Deployment hardening: validated split-host/public-URL settings, explicit packaged model-profile declarations, startup/readiness health endpoints, model-endpoint offline policy checks, pgvector readiness checks, container health probes, and operator-facing package docs.
...
- Runtime health now carries severity and remediation guidance across API responses, package scripts, and the fixed developer console; it also declares the packaged generator/embedding/accelerator profile so operators can verify deployment intent before deeper debugging, and the worker refuses to enter its loop when startup or live readiness checks fail.
