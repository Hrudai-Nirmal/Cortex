...
worker_startup_health="$(read_json "$CORTEX_QUERY_HOST" "/health/worker-startup")"
...
assert_contains "$worker_startup_health" '"status":"ready"' "worker-startup API status"
assert_contains "$worker_startup_health" '"blockingPhase":"none"' "worker-startup API blocking phase"
query_contract="$(read_json "$CORTEX_QUERY_HOST" "/v1/chat/contracts/v1")"
request_schema="$(read_json "$CORTEX_QUERY_HOST" "/v1/chat/contracts/v1/schemas/request")"
response_schema="$(read_json "$CORTEX_QUERY_HOST" "/v1/chat/contracts/v1/schemas/response")"
...
assert_contains "$query_contract" '"requestSchemaPath":"/v1/chat/contracts/v1/schemas/request"' "query contract request schema path"
assert_contains "$query_contract" '"responseSchemaPath":"/v1/chat/contracts/v1/schemas/response"' "query contract response schema path"
...
assert_contains "$request_schema" '"title":"ChatCompletionRequestSchema"' "query request schema title"
assert_contains "$request_schema" '"messages"' "query request schema messages property"
assert_contains "$request_schema" '"cortex"' "query request schema cortex property"
assert_contains "$request_schema" '"const":false' "query request schema stream const"
assert_contains "$response_schema" '"title":"ChatCompletionResponseSchema"' "query response schema title"
assert_contains "$response_schema" '"x_cortex"' "query response schema cortex property"
assert_contains "$response_schema" '"ExternalQueryMetadataSchema"' "query response schema metadata definition"
if [ "$CORTEX_QUERY_SURFACE_MODE" = "bundled" ]; then
